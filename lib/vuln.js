...
class Vuln {
  ...
  load (cached, packument) {
    ...
    if (this[_packument])
      throw new Error('vuln object already loaded')
  ...
  [_testVersion] (version, spec) {
    ...
    // search for a version that is valid and not vulnerable
    const paku = this[_source][_packument]
    for (const v of this[_source].versions) {
      const satisfies = semver.satisfies(v, spec, semverOpt)
      if (!satisfies)
        continue
      const vuln = semver.satisfies(v, avoid, semverOpt)
      if (!vuln) {
        memo.set(spec, false)
        return false
      }
    ...
    // XXX it *might* be a good idea to check if the spec is a subset
    // of the source.range, but erring on the side of "do not use" is
